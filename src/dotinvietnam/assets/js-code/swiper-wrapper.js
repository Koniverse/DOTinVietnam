(
	function( $ ) {
		...
		$.fn.DreambitcitySwiper = function( options ) {
			var defaults = {};
			...
			this.each( function() {
				...
				$swiper.on( 'init', function() {
					...
					visibleSlide.addClass( 'dotinvietnam-slide-active' );
				} );
				...
				$swiper.on( 'slideChangeTransitionStart', function() {
					var slides = $swiper.$wrapperEl.find( '.swiper-slide' );
					slides.removeClass( 'dotinvietnam-slide-active' );
				} );
				...
				$( document ).trigger( 'DreambitcitySwiperInit', [ $swiper, $slider, swiperOptions ] );
			} );
