(
	function( $ ) {
		...
		var DreambitcityGridPlugin = function( $el, options ) {
			this.$el = $el;
			var $grid = $el.find( '.dotinvietnam-grid' );
			...
			this.init = function() {
				...
				$el.on( 'DreambitcityQueryEnd', function( event, el, $items ) {
					plugin.update( $items );
				} );
			};
				...
				if ( $.fn.laziestloader ) {
					...
					if ( llImages.length > 0 ) {
						llImages.laziestloader( {}, function() {
							$( this ).unwrap( '.dotinvietnam-lazy-image' );
			...
			this.calculateMasonrySize = function( options = {} ) {
				...
				if ( oldActiveColumns !== plugin.activeColumns ) {
					$( document.body ).trigger( 'DreambitcityGridLayoutColumnsChange', [
						$el,
						oldActiveColumns,
				...
				setTimeout( function() {
					...
					$el.trigger( 'DreambitcityGridLayoutResized' );
				}, 250 );
			};
		...
		const namespace = 'dotinvietnamGridLayout';

		$.fn.extend( {
			DreambitcityGridLayout: function( args, update ) {
				...
				return this.each( function() {
					...
					if ( instance ) { // Already created then trigger method.
						...
						instance = new DreambitcityGridPlugin( $el, args );
